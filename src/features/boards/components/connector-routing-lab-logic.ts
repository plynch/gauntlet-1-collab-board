...
export function getAnchorPoint(shape: Shape, anchor: Anchor): Point {
...
export function toBounds(shape: Shape, padding = 0): Bounds {
  return {
...
export function pathIntersections(points: Point[], bounds: Bounds): number {
  let count = 0;
...
export function buildBestRoute(circle: Shape): {
  toAnchor: Anchor;
  route: Point[];
  overlapsCircle: boolean;
} {
  const fromPoint = getAnchorPoint(RECT_SHAPE, "bottom");
  ...
  circleAnchors.forEach((anchor) => {
    ...
    const score =
      overlapCount * 1_000_000 + pathLength(route) + Math.max(0, route.length - 2) * 120;
    if (score < bestScore) {
      bestScore = score;
      bestAnchor = anchor;
      bestRoute = route;
